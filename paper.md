...
We end by discussing opportunities for future work and how we can make CSS selectors more accessible to end-users ([@sec:conclusion]).
...
One aspect of future work is saving the list of all selectors that fufil the criteria and making them available to users to view and pick from via the formula. This would be similar to Mayer et el's user interaction model called *program navigation* [@mayer2015] that gives users the opportunity to navigate all valid, synthesized programs and pick the best one.
...
A key limitation of this approach is that users need to understand how CSS selectors work. Because the formula language utilizes pure funcitional semantics, users can iterate on CSS selectors as many times as they need to without having to worry about side effects. This makes the authoring of CSS selectors more accesible but more work remains for us to make this fully accessible to end-users. 
...
Our second evaluation of Joker is a user study which provides insights on what aspects of Joker work well and what don't. Because of this, successful completion of tasks did not equal validation of Joker. It did however inform us about usability issues that limit its effectiveness.
...
The main area of future work involves providing feedback to users about when demonstrations are not sufficient. Otherwise, users won't know to switch to formulas to take advantage of their increased expressiveness. FlashProg [@mayer2015] and Wrangler [@kandel2011], whose interfaces also have tables, offer clues for how to provide feedback about the execution of the program synthesized from demonstrations. Another important area involves making the formula language more accesible to end-users not familiar with CSS selectors. Again, FlashProg offers some clues through its use of high level descriptions to give meaning to synthesized programs. One avenue for associating meaning with CSS selectors could be by extracting sementic web content as seen in systems like Thresher [@hogue2005].
