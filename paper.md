...
secPrefix:
    ...
    - "Sections"
abstract: |
  Many websites on the internet do not meet the exact needs of all of their users. Prior research has resulted in systems that enable anyone to change the behavior of a website without having to perform traditional programming. However, these systems have a divide between the extraction and augmentation stages, which are central to their interaction models, that is a significant barrier to their usability.

  In this paper, we present a new interaction model for web customization that is built on a spreadsheet formula language that encompasses both extraction and augmentation tasks and has programming-by-demonstration (PBD) interface which makes it easy for anyone to program in. This interaction model allows users to seamlessly move between extraction and augmentation, breaking the divide between the two and resulting in a more iterative workflow that matches the natural progression of real-world web customization.

  To illustrate our unified interaction model for web customization, we implement it as a browser extension called Joker. Through concrete examples, a small user study and a Cognitive Dimensions of Notation analysis, we show how and why Joker can enable users to complete useful web customizations across a variety of websites in a more flexible manner.
...
Web customization systems have two main stages. The extraction stage involves getting structured data from the website into a tabular format. The augmentation stage involves augmentation of the tabular data by adding derived columns, performing operations such as sorting and filtering or both. In Sifter, the prices of items in a shopping catalog can be extracted to augment the website with a filtering interface. In Vegemite, the addresses of homes in a housing catalog can be extracted and augmented with walk scores. In Wildcard, the prerequisites of a course in a course catalog can be extracted and augmented with an indictor for whether the course has a certain prerequisite. While these systems provide a variety of mechanisms for customization, they share a common design limitation: there is a rigid separation between the extraction stage and the augmentation stage. The entire extraction stage always has to happen before the augmentation stage.

This separation between extraction and augmentation is an important barrier to usability. A user study [@lin2009] of Vegemite wrote that “it was confusing to use one technique to create the initial table, and another technique to add information to a new column.” The creators of Sifter similarly reported [@huynh2006] that “the necessity for extracting data before augmentation could take place was poorly understood, if understood at all.” In Wildcard, end-users cannot augment a website unless a programmer has written and shared extraction code for that website in Javascript. In general, this interaction model assumes a sequential workflow where users first extract all the data they need, and then perform all their desired customizations. In reality, however, users prefer a more iterative process where extraction and augmentation are interleaved (as demonstrated by these prior user studies and our own study in this work).

In this paper, we present a new approach to web customization that combines extraction and augmentation with into a unified interaction model. Our key idea is to develop a domain specific language (DSL) that encompasses both extraction and augmentation tasks, along with a programming-by-demonstration (PBD) interface that makes it easy for end-users to program in the language. This unified interaction model allows end-users to seamlessly move between extraction and augmentation, resulting in a more iterative workflow that matches the natural progression of real-world web customization.
...
**A Unified Formula Language**: Wildcard’s formula language only supported primitive values like strings and numbers. Joker extends this language by introducing Document Object Model (DOM) elements as a new type of value, and adding a new set of formulas for performing operations on them. This includes querying elements with Cascading Stylesheet Selectors (CSS) selectors and traversing the DOM. With this approach, a single formula language is used to express both extraction and augmentation tasks, even within a single formula expression.

**A PBD Interface**: Directly writing extraction formulas can be challenging for end-users, so Joker also implements a PBD interface that uses demonstrations to synthesize extraction formulas. Demonstrations have been used to synthesize data extraction programs in a number of prior systems. However, a key property of our design is that the extraction program synthesized from the demonstration is made visible to the user as a spreadsheet formula that can be edited and executed using pure functional semantics.
...
To concretely illustrate the user experience of our tool, we present a scenario of customizing eBay, a popular online marketplace. The goal of the user, Jen, is to filter out search results that are sponsored, i.e. product listings that a seller paid to promote. On the webpage, sponsored listings are marked with a "Sponsored" label that Jen can scrape, then she can sort the scraped labels to move all sponsored listings to the bottom of the page. @fig:ebay shows accompanying screenshots.
...
This formula reveals how Joker is actually scraping this column's data behind the scenes: it calls a query selector on each listing (i.e. each `rowElement`) on the page. All data that is scraped into the table is stored as functions that act on the elements on the page, instead of as static text, based on our design principle of **functional reactive programming**. Joker exposes this formula to the user to provide an example for how scraping is achieved with formulas that the user can edit to scrape other elements.
...
In this section, describe Joker's formula language include a table of currently available formulas. Then, we outline the *wrapper induction* [@kushmerick2000] algorithm that Joker's PBD interface uses to synthesize the row and column selectors presented in formulas. 
...
This formula is used to represent column value extractions. `this` is a special keyword that references a hidden column containing the DOM elements that correspond to the rows of the data set. `selector` is the synthesized CSS selector that specifies the column element. 
...
*Plausibility*. An element `R` is a plausible row element if 1) it is in the parent path of the column element `C` containing the demonstrated column value `V` and 2) the CSS selector $S_{column}$ of element `C` only identifies `C` when applied to `R`.
...
Joker's use of FRP makes it easier for users to program in its formula language. With traditional programming, the eBay example in [@sec:examples] in which a user extracts every listing's "Sponsored" label would be much more complicated. The user would need to understand programming constructs such as state, variables, looping and data flow in order to write a program to scrape the label. In Joker, all a user needs to specify is the CSS selector that identifies the element containing the label. Joker takes care of managing state, variables, looping and data flow!
...
Our second evaluation of Joker is a user study that was focused on providing insights on barriers to its usability.
...
All participants attempted all the tasks. We started each session with a description of Joker and provided a brief tutorial of its main features on a website not used for the tasks. There was no time limit for tasks but we provided hints whenever a participant had exhausted the knowledge available to them.
