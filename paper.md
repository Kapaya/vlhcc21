...
Millions of people use browser extensions and user scripts to customize their experience of the web. Unfortunately, creating these customizations requires programming knowledge. To make web customization more accessible to end users without programming experience, researchers have developed end-user customization tools like Thresher [@hogue2005], Sifter [@huynh2006], Vegemite [@lin2009], and Wildcard [@litt2020; @litt2020b]. A common part of using these customization tools is _web scraping_: extracting structured data from a webpage. Web scraping solutions in these tools must trade off between two conflicting goals. On the one hand, they must offer automation and ease of use to be accessible to end users who aren't familiar with web development. On the other hand, they must be powerful enough to operate accurately on the large variety of websites that exist in the wild.

In this paper, we propose a new model for web scraping that aims to satisfy both of these goals. At its core lies a simple formula language for expressing data extractions from HTML pages. The pure functional semantics of this language make it accessible to end users, and it also bridges the gap between data extraction and lightweight data transformation by accommodating both tasks in a single language.

Furthermore, we have developed a user interface that allows data extractions to be specified using *programming-by-demonstration*. The user can directly click on elements in the page; our tool automatically synthesizes formulas in our scraping language. Crucially, these formulas are visible to the user and can be directly edited, enabling users to specify complex scraping operations that cannot be described by demonstration alone. The tool also displays live intermediate results as the user performs the scraping process, allowing for a tight feedback loop that supports exploratory scraping and correcting errors throughout the process.

We have implemented these ideas as an extension to Wildcard, an end-user customization tool developed by Litt et al.  [@litt2020; @litt2020b]. Wildcard adds a spreadsheet-like table view to websites and establishes a bidirectional synchronization between the table and the page, allowing users to customize the webpage by manipulating the table view. However, end users were limited to customizing a small selection of websites, because Wildcard required programmers to write custom scraping code in Javascript mapping from a particular webpage to the data table. We have extended Wildcard with an end-user scraping system based on the above ideas, enabling end users to customize a far greater number of websites, and also making the system more convenient even for programmers who are familiar with web scraping.

<some kind of transition to contributions... maybe list out the design principles here?>
